...
async function sendMaliciousAccessWebhook(request: NextRequest, session: any, machineName: string) {
    ...
    try {
        ...
        const embed = {
            ...
            description: "Tentativa de iniciar máquina Azure sem autorização",
            color: 0xFF0000,
            fields: [
                {
...
export async function POST(req: NextRequest) {
    try {
        ...
        const poller = await computeClient.virtualMachines.beginStart(resourceGroupName, name);
        await poller.pollUntilDone();

        return NextResponse.json(
            {
                message: "Machine successfully started",
                support: '@dump.ts'
            },
            { status: 200 }
        );
        ...
        return NextResponse.json(
            {
                error: "Error when starting machine",
                support: '@dump.ts'
            },
            { status: 500 }
        );
